...
Conman is what should hopefully be me perfect in-browser presentation software.

The goals are ultimately to have each slide be as big as it can be, so that you don't have worry about
custom screens, font-sizes, or maximizing space.

It does this by compiling your presentation into an HTML file that, when loaded, will execute the Conman
...
This slide is for showing command-line invocations and results.  It's currently implemented in a pretty inflexible way, but the idea is that each command-line command is a line starting with a <tt>></tt> or <tt>%</tt>.  The line following is the output of that command.  When you play the presentation, the commands are "typewritered" out, and the results displayed automatically.

<b>This currently only works if you every line as a command and every other line as a result.  Sorry</b>It's currently implemented in a pretty inflexible way, but the idea is that each command-line command is a line starting with a <tt>></tt> or <tt>%</tt>.  The line following is the output of that command.  When you play the presentation, the commands are "typewritered" out, and the results displayed automatically.

<b>This currently only works if you every line as a command and every other line as a result.  Sorry</b>
